...
export default function CommandeLigneFreelance({ commande }: { commande: CommandeLigne }) {
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-foreground/50">
            {commande.reference} · Client : {commande.clientNom} · Échéance : {commande.dateEcheance}
          </p>
        </div>
      </div>
      ...
      <div className="flex items-center gap-4">
        ...
        {statut === "EN_ATTENTE" && (
          <div className="flex gap-2">
            <button
              onClick={() => envoyerAction("accepter")}
              disabled={chargement}
            ...
            <button
              onClick={() => envoyerAction("refuser")}
              disabled={chargement}
              className="rounded-full bg-muted px-3 py-1.5 text-xs font-semibold text-foreground/70 hover:bg-red-100 hover:text-red-600 disabled:opacity-50"
            >
              Refuser
            </button>
          </div>
        )}
        {(statut === "EN_COURS" || statut === "REVISION_DEMANDEE") && (
          <button
            onClick={() => envoyerAction("livrer")}
            disabled={chargement}
            className="rounded-full bg-accent px-3 py-1.5 text-xs font-semibold text-accent-foreground hover:bg-accent-dark disabled:opacity-50"
          >
            Livrer le travail
          </button>
        )}
